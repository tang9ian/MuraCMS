...
<cffunction name="init" output="false">
	...
	<cfif structKeyExists(arguments,"data") and not (isSimpleValue(arguments.data) and not len(arguments.data))>
		...
		<cfelse>
			...
			<cfset setEvent(createObject("component","com.mura.event").init(initArgs).setValue('MuraScope',this))>
		</cfif>
	</cfif>
...
<cffunction name="getEvent" output="false" returntype="any">
	<cfif not isObject(variables.instance.event)>
		...
		<cfelse>
			<cfset variables.instance.event=createObject("component","com.mura.event").init($=this)>
		</cfif>
	</cfif>
...
<cffunction name="getGlobalEvent" output="false" returntype="any">
	...
	<cfelse>
		...
		<cfset request.muraGlobalEvent=createObject("component","com.mura.event").init(temp)>
...
<cffunction name="each">
	...
	<cfelseif isQuery(arguments.collection)>
		<cfset queryIterator=createObject("component","com.mura.iterator.queryIterator")>
