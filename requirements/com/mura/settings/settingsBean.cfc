...
--->
<cfcomponent extends="com.mura.bean.beanExtendable" output="false">
...
<cffunction name="getRBFactory" returntype="any" access="public" output="false">
	...
	<cfif not isObject(variables.instance.rbFactory)>
		<cfset tmpFactory=createObject("component","com.mura.resourceBundle.resourceBundleFactory").init(application.rbFactory,"#expandPath('/#variables.configBean.getWebRootMap()#')#/#variables.instance.displayPoolID#/includes/resourceBundles/",getJavaLocale())>
		<cfset themeRBDir=expandPath(getThemeIncludePath()) & "/resourceBundles/">
		<cfif directoryExists(themeRBDir)>
			<cfset variables.instance.rbFactory=createObject("component","com.mura.resourceBundle.resourceBundleFactory").init(tmpFactory,themeRBDir,getJavaLocale()) />
		<cfelse>
...
if(not isObject(arguments.$)){
	if(not isObject(variables.instance.themeRenderer)){
		...
		if(fileExists(expandPath(getThemeIncludePath()) & "/contentRenderer.cfc")){
			...
			variables.instance.themeRenderer=createObject("component","com.mura.cfobject")
			.injectMethod("$",arguments.$)
			.injectMethod("mura",arguments.$)
			.injectMethod("event",arguments.$.event())
			.init(event=arguments.$.event(),$=arguments.$,MURA=arguments.$);
	...
	if(fileExists(expandPath(getThemeIncludePath()) & "/contentRenderer.cfc")){
		...
		return createObject("component","com.mura.cfobject")
			.injectMethod("$",arguments.$)
			.injectMethod("mura",arguments.$)
			.injectMethod("event",arguments.$.event())
			.init(event=arguments.$.event(),$=arguments.$,MURA=arguments.$);
