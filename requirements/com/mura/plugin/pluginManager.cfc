...
<cfset variables.zipTool=createObject("component","com.mura.Zip")>
	...
	<cflock name="pluginDiscovery#application.instanceID#" type="exclusive" timeout="200">
		...
		<cfloop query="rsRequirements">
			...
			<cfelseif listLast(rsRequirements.name,".") eq 'zip'>
				...
				<cfelse>
					<cfset item="#baseDir#/#rsRequirements.name#">
					<cfset rsCheckDiscoveredPlugin=createObject("component","com.mura.Zip").list(item)>
...
<cfargument name="cache" required="true" default="true">
	...
	<cfif not arguments.cache or not structKeyExists(variables.pluginConfigs,"p#rs.pluginID#")>
		<cfset pluginConfig=createObject("component","com.mura.plugin.pluginConfig")>
...
<cfargument name="moduleID" required="true" default="" type="any">
	<cfset var siteID="">
	<cfif variables.utility.checkForInstanceOf(arguments.currentEventObject,"com.mura.MuraScope")>
	...
	<cftry>
		...
		<cfif not isObject(arguments.event)>
			<cfif isStruct(arguments.event)>
				<cfset arguments.event=createObject("component","com.mura.event").init(arguments.event)/>
			<cfelse>				
				<cfif structKeyExists(request,"servletEvent")>
					<cfset arguments.event=request.servletEvent />
				<cfelse>
					<cfset arguments.event=createObject("component","com.mura.event").init()/>
				</cfif>
			</cfif>
		</cfif>
		
		<cfif variables.utility.checkForInstanceOf(arguments.event,"com.mura.MuraScope")>
...
<cfargument name="siteid" required="true" default="">
	...
	<cfif not structKeyExists(variables.cacheFactories,arguments.siteid)>
		<cfset variables.cacheFactories[arguments.siteid]=createObject("component","com.mura.cache.cacheFactory").init(isSoft=false)>
	</cfif>
